...
class CFValidationCallback(pl.callbacks.Callback):
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        if not ((pl_module.current_epoch < 500 and pl_module.current_epoch % 50 == 0) or (pl_module.current_epoch >= 500 and pl_module.current_epoch % 250 == 0)):
            return
    ...
    def on_train_end(self, trainer, pl_module):
        ...
        if len(self.state["val_cf_rmse_known"]) > 0:
            ...
            plt.plot(self.state["epoch"], self.state["val_cf_rmse_unknown"], label=f"Unknown {self.side}s", color="blue")
            plt.plot(self.state["epoch"], self.state["val_cf_rmse"], label=f"All {self.side}s", color="purple")
        plt.xlabel("Compressor Epoch")
        plt.ylabel("Validation RMSE")

        plt.ylim(0.800, 0.820)
        # Tick every 0.001 increment
        plt.yticks([i/1000 for i in range(800, 820, 2)])
...
class CFValidationCallbackSimultaneous(pl.callbacks.Callback):
    def __init__(self, cf_model, validation_data, ids = None):
        ...
        self.state = {"val_cf_rmse_user": [], "val_cf_rmse_item": [], "val_cf_rmse": [], "val_cf_rmse_og": [], "epoch": [], "val_cf_rmse_ui": [], "val_cf_rmse_u": [], "val_cf_rmse_i": [], "val_cf_rmse_new": []}
        ...
        self.val_dataloaders = [
            torch.utils.data.DataLoader(
                DyadicRegressionDataset(self.val_data[self.val_data["user_id"].isin(self.known_users) & self.val_data["item_id"].isin(self.known_items)].reset_index(drop=True)), batch_size=2**14, shuffle=False, num_workers=4, persistent_workers=True
            ),
            torch.utils.data.DataLoader(
                DyadicRegressionDataset(self.val_data[self.val_data["user_id"].isin(self.known_users) & self.val_data["item_id"].isin(self.unknown_items)].reset_index(drop=True)), batch_size=2**14, shuffle=False, num_workers=4, persistent_workers=True
            ),
            torch.utils.data.DataLoader(
                DyadicRegressionDataset(self.val_data[self.val_data["user_id"].isin(self.unknown_users) & self.val_data["item_id"].isin(self.known_items)].reset_index(drop=True)), batch_size=2**14, shuffle=False, num_workers=4, persistent_workers=True
            ), 
            torch.utils.data.DataLoader(
                DyadicRegressionDataset(self.val_data[self.val_data["user_id"].isin(self.unknown_users) & self.val_data["item_id"].isin(self.unknown_items)].reset_index(drop=True)), batch_size=2**14, shuffle=False, num_workers=4, persistent_workers=True
            )
        ]
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        trainer_cf = pl.Trainer(accelerator="auto", enable_progress_bar=False, gpus=1)
        cf_model_validation_losses = trainer_cf.validate(self.cf_model, dataloaders=self.val_dataloaders, verbose=False)

        pl_module.log("val_cf_rmse_ui", cf_model_validation_losses[0]["val_rmse/dataloader_idx_0"], on_step=False, on_epoch=True, prog_bar=True)
        pl_module.log("val_cf_rmse_u", cf_model_validation_losses[1]["val_rmse/dataloader_idx_1"], on_step=False, on_epoch=True, prog_bar=True)
        pl_module.log("val_cf_rmse_i", cf_model_validation_losses[2]["val_rmse/dataloader_idx_2"], on_step=False, on_epoch=True, prog_bar=True)
        pl_module.log("val_cf_rmse_new", cf_model_validation_losses[3]["val_rmse/dataloader_idx_3"], on_step=False, on_epoch=True, prog_bar=True)
        pl_module.log("val_cf_rmse", cf_model_validation_losses[0]["val_rmse"], on_step=False, on_epoch=True, prog_bar=True)
