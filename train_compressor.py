...
def compute_rmse(model, train_data, test_data, user_ids = None, item_ids = None):
    ...
    if user_ids is not None:
        ...
        test_dataloaders.append(torch.utils.data.DataLoader(
            DyadicRegressionDataset(test_data_known), batch_size=2**14, shuffle=False, num_workers=4, persistent_workers=True
        ))
        ...
        torch.utils.data.DataLoader(
            DyadicRegressionDataset(test_data_unknown), batch_size=2**14, shuffle=False, num_workers=4, persistent_workers=True
        )
            )
...
def train_compressor(
    ...
    data_module = EmbeddingDataModule(embeddings, ids=ids, batch_size=256, num_workers=4)
    ...
    dataloader_known = torch.utils.data.DataLoader(
        DyadicRegressionDataset(cf_val_data_known), batch_size=2**14, shuffle=False, num_workers=4, persistent_workers=True
    )

    dataloader_unknown = torch.utils.data.DataLoader(
        DyadicRegressionDataset(cf_val_data_unknown), batch_size=2**14, shuffle=False, num_workers=4, persistent_workers=True
    )
    ...
    trainer = pl.Trainer(
        gpus=1, enable_progress_bar=not is_tuning, max_time="00:03:00:00", enable_checkpointing=False, logger=False, enable_model_summary=False, callbacks=[val_cf_callback], num_sanity_val_steps=-1   
    )
...
if __name__ == "__main__":
    ...
    compressed_user_embeddings = train_compressor(user_embeddings, ORIGIN_DIM, TARGET_DIM, 1e-3, 0, cf_model=copy.deepcopy(model_original), cf_val_data=test_data_og, side="user", ids=user_ids)
    compressed_item_embeddings = train_compressor(item_embeddings, ORIGIN_DIM, TARGET_DIM, 1e-3, 0, cf_model=copy.deepcopy(model_original), cf_val_data=test_data_og, side="item", ids=item_ids)
